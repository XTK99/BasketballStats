...
async function saveTeamStats(teamStatsArray, gameId) {
  if (!Array.isArray(teamStatsArray) || !teamStatsArray.length)
    throw new Error("teamStatsArray must be a non-empty array");
  ...
  const values = [];
  const placeholders = teamStatsArray.map((stats, i) => {
    console.log("Processing team stats for gameId", gameId, stats);
